...
// Fill remaining roles with Full Stack template
['ML Engineer', 'Product Manager', 'UI/UX Designer', 'Mobile Developer', 'Cloud Architect'].forEach(role => {
  if (!ROADMAP_TEMPLATES[role]) ROADMAP_TEMPLATES[role] = ROADMAP_TEMPLATES['Full Stack Developer'];
});
...
export default async function handler(req: Request, res: Response) {
  ...
  const phases = template.map(phase => ({ ...phase, completed: false }));

  const roadmap = { id: generateId(), userId, targetRole: role, phases, createdAt: new Date().toISOString() };
